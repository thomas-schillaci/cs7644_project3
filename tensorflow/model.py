...
class CNNModel:
	...
	def __init__(self, height, width, channels, classes, learning_rate, mean, std):
		...
		self.x = (self.X - self.mean) / self.std  # TODO
		...
		self.conv1 = tf.keras.layers.Conv2D(6, [6, 6], strides=(2, 2), padding='same', activation=tf.nn.relu,
											name='conv_1')(self.x)
		...
		self.conv2 = tf.keras.layers.Conv2D(12, [5, 5], strides=(2, 2), padding='same', activation=tf.nn.relu,
											name='conv_2')(self.norm1)
		...
		self.conv3 = tf.keras.layers.Conv2D(24, [4, 4], strides=(2, 2), padding='same', activation=tf.nn.relu,
											name='conv_3')(self.norm2)
