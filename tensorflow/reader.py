...
class PNGReader:
	...
	def load_data(self):
		...
		indices = np.array([k for k in range(len(self.x_train))])
		np.random.shuffle(indices)
		x2 = []
		y2 = []

		for index in indices:
			x2.append(self.x_train[index])
			y2.append(self.y_train[index])

		x2=np.array(x2)
		y2=np.array(y2)

		assert(x2.shape==np.shape(self.x_train))
		assert(y2.shape==np.shape(self.y_train))
	...
	def compute_mean(self):
		...
		self.mean = np.mean(self.x_train, axis=(0, 1, 2))  # TODO
		self.std = np.std(self.x_train, axis=(0, 1, 2))  # TODO
