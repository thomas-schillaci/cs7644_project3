...
class Train:
	def __init__(self):
		...
		self.dataset = PNGReader(self.settings.train_file, self.settings.train_root, self.settings.val_file,
	...
	def train(self):
		...
		with tf.compat.v1.Session() as self.sess:
			self.train_writer = tf.compat.v1.summary.FileWriter(self.settings.tensorboard_dir + '/train',
																self.sess.graph)
			...
			for step in range(self.settings.max_iter):
				...
				if step % 25 == 0:  # Eval on validation-set
					VBX, VBY = self.sampler.sample_val_batch(50)
